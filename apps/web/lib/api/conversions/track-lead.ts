...
export const trackLead = async ({
  ...
  // TODO: Maybe we can replace this to rely only on Postgres directly since we're checking the customer above?
  if (mode !== "deferred") {
  ...
  if (!isDuplicateEvent) {
    ...
    // if the customer doesn't exist in our Postgres DB yet, upsert it
